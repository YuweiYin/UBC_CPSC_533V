...
#
# According to Pontryagin's maximum principle it is optimal to fire engine full throttle or
# turn it off. That is the reason this environment is OK to have discreet actions (engine on or off).
#
...
NOOP = 1  # do not fire main engine, do not steer
...
class LunarLander(gym.Env):
    ...
    def _render(self, mode="rgb_array", close=False):
        ...
        self.viewer.draw_polyline(
            [(0, 0.05 * VIEWPORT_H / SCALE), (clock_prog * VIEWPORT_W / SCALE, 0.05 * VIEWPORT_H / SCALE)],
...
def heuristic(env, s):
    ...
    angle_ = (angle_targ - s[4]) * 0.5 - (s[5]) * 1.0
    # print("angle_targ=%0.2f, angle_=%0.2f" % (angle_targ, angle_))

    # PID controller: s[1] vertical coordinate s[3] vertical speed
    hover_ = (hover_targ - s[1]) * 0.5 - (s[3]) * 0.5
    # print("hover_targ=%0.2f, hover_=%0.2f" % (hover_targ, hover_))

    if s[6] or s[7]:  # legs have contact
        angle_ = 0
        hover_ = -(s[3]) * 0.5  # override to reduce fall speed, that is all we need after contact

    a = np.array([hover_ * 20 - 1, -angle_ * 20])
    a = np.clip(a, -1, +1)
    return a
