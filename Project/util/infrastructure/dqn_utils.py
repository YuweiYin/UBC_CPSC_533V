...
def get_env_kwargs(env_name):
    ...
    elif env_name == "HalfCheetah-v4":  # TODO: Action Discretization
        def halfcheetah_empty_wrapper(env):
            return env

        kwargs = {
            "optimizer_spec": halfcheetah_optimizer(),
            "q_func": create_lander_q_network,
            "replay_buffer_size": 50000,
            "batch_size": 32,
            "gamma": 1.00,
            ...
            "grad_norm_clipping": 10,
            "num_timesteps": 500000,
            "env_wrappers": halfcheetah_empty_wrapper,
            "seed": 42,
        }
        kwargs["exploration_schedule"] = halfcheetah_exploration_schedule(kwargs["num_timesteps"])
...
def lander_optimizer():
    return OptimizerSpec(
        ...
        learning_rate_schedule=lambda epoch: 1e-3,  # keep the init learning rate
    )
...
def halfcheetah_optimizer():
    return OptimizerSpec(
        ...
        learning_rate_schedule=lambda epoch: 5e-3,  # keep the init learning rate
    )


def halfcheetah_exploration_schedule(num_timesteps):
    return PiecewiseSchedule(
        [
            (0, 1),
            (num_timesteps * 0.1, 0.02),
...
def initialize_interdependent_variables(session, vars_list, feed_dict):
    ...
    while len(vars_left) > 0:
        ...
        if len(new_vars_left) >= len(vars_left):
            # This can happen if the variables all depend on each other, or more likely if there is
            # another variable outside of the list, that still needs to be initialized. This could be
            # detected here, but life is finite.
...
class MemoryOptimizedReplayBuffer(object):
    ...
    def store_frame(self, frame, num_actions: int = 1):
        ...
        if self.obs is None:
            ...
            # self.action = np.empty([self.size] + [num_actions], dtype=np.float32)  # multiple actions
    ...
    def store_effect(self, idx, action, reward, done):
        ...
        up into two functions is so that once can call `encode_recent_observation` in between.
        ...
        reward: float
            Reward that was received when the action was performed.
        done: bool
