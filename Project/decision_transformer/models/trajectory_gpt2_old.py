...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False):
        ...
        if config.add_cross_attention:
            self.cross_attn = Attention(hidden_size, n_ctx, config, scale, is_cross_attention=True)
...
class GPT2DoubleHeadsModelOutput(ModelOutput):
    ...
    Args:
        ...
        past_key_values (:obj:`List[torch.FloatTensor]`, `optional`, returned when ``use_cache=True`` is passed or
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        # we need to make broadcast-able to [batch_size, num_heads, seq_length, seq_length]
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            # Model Parallel: If it is the last layer for that device, put things on the next device
            if self.model_parallel:
                for k, v in self.device_map.items():
