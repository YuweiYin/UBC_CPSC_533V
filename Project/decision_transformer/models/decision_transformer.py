...
class DecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        # note: we don't predict states or rewards for the paper
    ...
    def get_action(
        ...
        if self.max_length is not None:
            ...
            rewards_to_go = torch.cat(
                [torch.zeros((rewards_to_go.shape[0], self.max_length-rewards_to_go.shape[1], 1), device=rewards_to_go.device), rewards_to_go],
                dim=1).to(dtype=torch.float32)
            timesteps = torch.cat(
